...
export default function Navbar() {
  ...
  return (
    ...
    >
      {/* Top Contact Bar */}
      <div className="hidden md:flex items-center justify-between px-16 py-1.5 border-b border-white/5 bg-ink/40 backdrop-blur-sm">
      ...
      <div
        className={`flex items-center justify-between transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
          isScrolled
            ? "px-8 md:px-16 py-2 bg-ink/95 backdrop-blur-md shadow-2xl"
            : "px-8 md:px-16 py-4 bg-ink/80 backdrop-blur-sm"
        }`}
      >
        <Link
          href="/"
          className="relative h-18 w-52 md:w-64 no-underline flex items-center"
        >
          <img 
            src="/logo.png" 
